...
5. **테스트 방법 제시** (중요!) ⭐ 신규 추가
   - 구현한 기능의 테스트 방법을 구체적으로 제시
   - 사용자가 직접 실행할 수 있는 단계별 가이드 작성
   - 예상 결과와 확인 포인트 명시
...
6. **Git 동기화** (반드시 이 순서로!)
   ...
   **중요**: 
   - `git add .`로 **모든 변경된 파일** 포함
   - 세션 파일뿐만 아니라 작업으로 수정된 모든 파일 동기화
   - workspace 파일 등 불필요한 파일은 .gitignore로 자동 제외됨
